...
struct Compare {
    bool operator()(int a, int b) {
        // 按照绝对值大小排序，绝对值大的优先级高
// 注意和sort函数区分开，compare函数是true的时候就把a放在后面
        return abs(a) < abs(b);
    }
};
...
struct Compare {
    bool operator()(Node* a, Node* b) {
        ...
        // 频率相同，按字符升序（叶子节点字符小的在前，内部节点字符视为大）
        // 如果 a 是内部节点（ch='\0'）且 b 是叶子，则 a 排在后面
        if (a->ch == '\0' && b->ch != '\0') return true; // a 在 b 后面
        if (b->ch == '\0' && a->ch != '\0') return false; // a 在 b 前面
        return a->ch > b->ch; // 都是叶子则字符大的在后
    }
};
