...
cpp
while (cin >> m && m != 0) {
    vector<int> nums(m);        // 新创建
    vector<int> in_result;      // 新创建
    vector<int> pre_result;     // 新创建
    // ...
}
当每次循环结束时，这些局部变量会自动销毁，它们的内存会被释放。然后在下一轮循环开始时，会重新创建新的vector实例。

这是完全正确的做法，有以下几个优点：

自动内存管理：不需要手动清空vector，循环结束时自动销毁

代码清晰：每轮循环都是独立的，不会受到前一轮数据的影响

避免错误：防止意外地重复使用旧数据


如果您在循环外部尝试访问循环内部定义的局部变量，比如：

cpp
int main() {
    int m;
    while (cin >> m && m != 0) {
        vector<int> nums(m);        // 局部变量
        vector<int> in_result;      // 局部变量
        vector<int> pre_result;     // 局部变量
        // ...
    } // 这里所有局部变量被销毁
    
    cout << nums.front();  // 错误！nums已经不存在了
    return 0;
}
这会导致编译错误，因为nums的作用域仅限于while循环内部。
...
生命周期：局部变量在离开作用域时自动销毁

在您的代码中：

nums、in_result、pre_result的作用域是while循环体内部

当程序执行到}时，这些变量就离开了作用域，被自动销毁

在循环外部，这些变量已经不存在了，所以无法访问
```
