...
```
  vector<int> l(n+1,0);
    ...
    for(int i=1;i<=n;i++){
        if(!sons[i].empty()){
            l[i]=sons[i][0];
            for(int j=0;j<sons[i].size()-1;j++){
                r[sons[i][j]]=sons[i][j+1];
            }
        }
    }
...
```
注意： 手动用 new 创建节点后，别忘了在程序结束前释放内存（或使用智能指针）。竞赛中如果节点数不多也可忽略。

# 数组版完全二叉树（适用于堆、线段树、淘汰赛等）
对于完全二叉树，可以直接用数组存储，下标从 1 开始，父子关系用索引计算：
- 节点 i 的左孩子：2*i
- 节点 i 的右孩子：2*i + 1
- 节点 i 的父节点：i / 2
```
...
// 假设总结点数为 2^n，存储从 1 到 n
vector<int> tree;   // 1-indexed

// 构建完全二叉树（示例：直接读入叶节点放到数组对应位置）
void buildCompleteTree(int n) {
    int size = 1 << n;          // 2^n 个叶节点
    tree.resize(2 * size);      // 最多 2*size-1 个节点
    for (int i = 0; i < size; ++i) {
        cin >> tree[size + i];  // 叶节点从 size 开始存放
    }
    // 从下往上建树（如线段树的 push_up）
    for (int i = size - 1; i >= 1; --i) {
        tree[i] = max(tree[2*i], tree[2*i+1]); // 示例：父节点取较大值
    }
}
对于竞赛中常见的淘汰赛/锦标赛树（求亚军），其实可以直接在原数组上划分左右半区，不需要显式建树，但若想用完全二叉树模拟：

cpp
int n, m; // m = 2^n
vector<int> a(m + 1);          // 1-indexed 存储每个叶节点的值（国家能力值）
// 无需实际建树，直接按范围找即可（见上一题的解法）
```
## 模板选择建议
- 题目给的是任意父子关系，且非完全二叉树 → 用指针版。
- 题目是完全二叉树（堆、线段树、淘汰赛树）→ 用数组版，下标计算高效且免去指针管理。
- 如果只是需要逻辑上的二叉树分区（如淘汰赛半区），往往可以不建树，用区间索引模拟，更简单高效。
